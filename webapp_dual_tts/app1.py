...
def generate_voice_cloning(text: str, reference_audio: str, tts_port: int) -> str:
    """
...
def process_task_background(task_id, text, video_path):
    ...
    try:
        ...
        cloned_audio, duration, tts_time = generate_voice_cloning(text, reference_audio, tts_port)
        print(f"   ✓ Voice clone ready: {cloned_audio} ({duration:.2f}s)")
        
        # Store TTS timing in task metadata
        with scheduler.lock:
